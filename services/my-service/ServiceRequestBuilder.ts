 ...
 */
export class ServiceRequestBuilder<T extends DeSerializers = DefaultDeSerializers> extends RequestBuilder<Service<T>, T> {
  /**
  ...
  getByKey(id: DeserializedType<T, 'Edm.Int32'>): GetByKeyRequestBuilder<Service<T>, T> {
    return new GetByKeyRequestBuilder<Service<T>, T>(this.entityApi, { ID: id });
  }
  ...
  getAll(): GetAllRequestBuilder<Service<T>, T> {
    return new GetAllRequestBuilder<Service<T>, T>(this.entityApi);
  }
  ...
  create(entity: Service<T>): CreateRequestBuilder<Service<T>, T> {
    return new CreateRequestBuilder<Service<T>, T>(this.entityApi, entity);
  }
  ...
  update(entity: Service<T>): UpdateRequestBuilder<Service<T>, T> {
    return new UpdateRequestBuilder<Service<T>, T>(this.entityApi, entity);
  }
  ...
  delete(idOrEntity: any): DeleteRequestBuilder<Service<T>, T> {
    return new DeleteRequestBuilder<Service<T>, T>(this.entityApi, idOrEntity instanceof Service ? idOrEntity : { ID: idOrEntity! });
  }
}
